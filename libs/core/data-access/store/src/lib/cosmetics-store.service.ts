...
export class CosmeticsStoreService {
  ...
  search(name: string) {
    ...
    this.http.get<CosmeticsSearchResponse>(`/api/getCosmetics?name=${encodeURIComponent(trimmed)}`)
      .subscribe({
        next: (response) => {
        ...
        error: (error: HttpErrorResponse) => {
          ...
          if (error.status === 404) {
            ...
            this._error.set('Locker search is currently unavailable. Please try again later.');
          }
        }
      });
  }
