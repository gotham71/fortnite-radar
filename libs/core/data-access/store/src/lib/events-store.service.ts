...
export class EventsStoreService {
  ...
  getAllEvents() {
    ...
    this.withLoading(this.http.get<TournamentResponse>(`/getAllEvents`))
      .subscribe({
        next: (response) => {
  ...
  getActiveEvents() {
    this.withLoading(this.http.get<TournamentResponse>(`/getActiveEvents`))
      .subscribe({
        next: (response) => {
  ...
  getWindowDetailsById(windowId: string) {
    this.withLoading(
      this.http.get<TournamentWindowResponse>(`/getWindowDetailsById?windowId=${windowId}`)
    ).subscribe({
      next: (response) => {
